...
<div id="paneI">
  ...
  <div id="pGoal" class="card hero">
    ...
    <div class="opts">
      ...
      <label class="chkbox" title="用真实高德 POI + AI 解析；调不通自动回退本地库"><input id="useLlm" type="checkbox"> 🌐 用真实数据</label>
...
function decCard(d,interactive){
  ...
  let h=`<div class="dcard ${resolved?'done':d.disposition}" data-dec="${d.id}">`;
...
async function start(){
  ...
  $('decisions').innerHTML=r.plan.decisions.map(d=>decCard(d,true)).join('');
...
async function submitAnswers(){
  ...
  if(!r.ok){alert(r.error);return;}
  $('decisions').innerHTML=r.plan.decisions.map(d=>decCard(d,false)).join('');
...
async function applyResolve(){
  ...
  $('v2Decisions').innerHTML=r.plan.decisions.map(d=>decCard(d,false)).join('')+renderTimeline(r.plan.timeline);
